...
The method found in `selfCheckQA.ts` and `qaWithSelfCheck.ts` check (using an LLM to detect a hallucination within the output) performed much better. There were fewer false positives, and a > 50% success rate in identifying hallucinations.
